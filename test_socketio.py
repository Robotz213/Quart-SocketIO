...
def on_connect(auth):
    ...
    send(
        json.dumps({
            h: request.headers[h] for h in request.headers.keys() if h not in ["Host", "Content-Type", "Content-Length"]
        })
    )
...
socketio.on_event("yet another custom namespace event", on_custom_event_test2, namespace="/test")
...
class TestSocketIO(unittest.TestCase):
    ...
    def test_connect_query_string_and_headers(self):
        client = socketio.test_client(
            app, query_string="?foo=bar&foo=baz", headers={"Authorization": "Bearer foobar"}, auth={"foo": "bar"}
        )
        received = client.get_received()
        self.assertEqual(len(received), 3)
        self.assertEqual(received[0]["args"], "connected")
    ...
    def test_connect_namespace_query_string_and_headers(self):
        client = socketio.test_client(
            app, namespace="/test", query_string="foo=bar", headers={"My-Custom-Header": "Value"}
        )
        received = client.get_received("/test")
        self.assertEqual(len(received), 3)
        self.assertEqual(received[0]["args"], "connected-test")
    ...
    def test_connect_class_based_query_string_and_headers(self):
        client = socketio.test_client(
            app, namespace="/ns", query_string="foo=bar", headers={"Authorization": "Basic foobar"}
        )
        received = client.get_received("/ns")
        self.assertEqual(len(received), 3)
        self.assertEqual(received[0]["args"], "connected-ns")
